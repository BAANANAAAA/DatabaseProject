...
function MainFrame(props) {
  ...
  const [users, setUsers] = useState([]) // This will hold the fetched user data
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        const response = await fetch(`/api/getMessage?userId=${userId}`)
        ...
        if (result.success) {
          const formattedUsers = result.data.map((message) => ({
            ...
            replies: [], // Assuming no nested replies for simplification
          }))
