...
function NewThread({ onThreadCreated }) {
  ...
  useEffect(() => {
    ...
    resetRelevantFields(tType) // Reset fields based on the type selected
  }, [tType])
  ...
  const resetRelevantFields = (type) => {
    // Set all optional fields to null initially
    setTargetName('')
    setTCreateTime('')
    setTReceiverID('')
    setToBlockID('')
    ...
    // Then, reset specific fields based on the type if necessary
    if (type !== 'SingleFriend' && type !== 'SingleNeighbor') {
      setFriends([]) // Clear friends if not relevant
    }
  }
