...
function NewThread({ onThreadCreated, userId, setShowForm }) {
  ...
  const [neighbors, setNeighbors] = useState([]) // State for storing neighbors
  const [selectedName, setSelectedName] = useState('') // State to store the selected friend or neighbor name

  useEffect(() => {
    if (tType === 'SingleFriend') {
      fetchFriends()
    } else if (tType === 'SingleNeighbor') {
      fetchNeighbors() // Fetch neighbors when Single Neighbor is selected
    }
  }, [tType, userId])
