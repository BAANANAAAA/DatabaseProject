...
function ViewMessage(props) {
  ...
  const handleResponse = async (AppID, accepted, type) => {
    ...
    // Determine the key for the application ID based on the type
    const appIDKey = type === 'friend' ? 'fAppID' : 'bAppID'

    try {
      // Construct the body with the dynamic application ID key
      ...
      if (response.ok) {
        fetchRequests() // Presumed function to refresh the list of requests
